...
def sim_wrank(N, p, k, rho, corstr, FDR, offset, nW,
              nsim_x, nsim_yx, wtype, toCSV=True, tag=''):
    if corstr == 'ar1':
    ...
    fname += '.csv'
    rslt = []
    vheader = ['k','N','p','rho','offset','nW', 'jx','jyx','FDR', 'fdp','tpr']
    vheader.extend(["sel{:d}".format(i) for i in range(p)])
    vfmt = ['%d', '%d', '%d','%.3f','%d','%d','%d','%d','%.3f', '%.18e', '%.18e']
...
if __name__ == "__main__":
    ...
    wtype = args.wtype
    nsim_x, nsim_yx, nW = (args.nsim_x, args.nsim_yx, args.nW)
    ...
    sim_wrank(N,p,k,rho,corstr,FDR,offset,nW,nsim_x,nsim_yx,wtype, tag=ftag)
